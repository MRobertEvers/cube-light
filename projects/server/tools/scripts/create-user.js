...
const { UserStore, USERNAME } = require(path.join(root, 'build/src/auth/UserStore'));
const { hashPassword, MIN_PASSWORD_LENGTH } = require(path.join(root, 'build/src/auth/passwords'));
...
async function main() {
	...
	if (!USERNAME.test(username)) {
		throw new Error('Usage: npm run create-user -- <username> (3 to 64 letters, digits, or . _ - @)');
	}
