...
function validBannerCrop(value: unknown): boolean {
	...
	return ['desktop', 'mobile'].every((variant) => {
		...
		return (
			...
			values.x >= 0 &&
			values.x <= MAX_MASKED_BANNER_X &&
			typeof values.y === 'number' &&
			Number.isFinite(values.y) &&
			values.y >= 0 &&
			values.y <= 1 &&
			typeof values.zoom === 'number' &&
			Number.isFinite(values.zoom) &&
			values.zoom >= 1 &&
			values.zoom <= 3
		);
	});
...
export function createRoutesDecksId(
	...
	app.get(routePath, async (req: Request<{ id: string }>, res: Response) => {
		...
		const cardInfosMapped = cards.reduce(
			(map, card) => {
				map[card.uuid] = card;
				return map;
			},
		...
		let bannerCard: DeckOverviewCardInfo | undefined = cardData.find(
			(card) => card.uuid === deck.BannerCardUuid
		);
		if (!bannerCard && deck.BannerCardUuid) {
			const [storedCard] = await cardDatabase.queryCardInfo([
				deck.BannerCardUuid
			]);
			if (storedCard) {
				[bannerCard] = await getDeckOverviewCardInfo(
					[deck.BannerCardUuid],
					cardDatabase,
					imageBaseUrl(req)
				);
		...
		res.send(
			JSON.stringify({
				...
				bannerCardUuid:
					deck.BannerCardUuid ??
					cardData.find((card) => card.art === icon)?.uuid ??
					null,
				bannerCard: bannerCard
					? {
							name: bannerCard.name,
							uuid: bannerCard.uuid,
							setCode: bannerCard.setCode,
							art: bannerCard.art ?? null
						}
					: null,
	...
	app.put(routePath, async (req: Request<{ id: string }>, res: Response) => {
		...
		if (bannerCardUuid !== undefined) {
			const [card] = await cardDatabase.queryCardInfo([
				bannerCardUuid as string
			]);
			if (!card) {
				res.sendStatus(400);
				return;
			...
			const deckUuids = new Set(
				(await database.getDeckCards(String(deck.DeckId))).map(
					(entry) => entry.Uuid
				)
			);
		...
		await database.updateDeckDetails(
			String(deck.DeckId),
			name.trim(),
			art,
			bannerCardUuid as string | undefined
		);
		res.sendStatus(204);
	});
	...
	app.get(
		...
		async (req: Request<{ id: string }>, res: Response) => {
			...
			const cards = await cardDatabase.queryCardInfo(
				deckCards.map((card) => card.Uuid)
			);
