...
const CURRENT_KEYS = [...LEGACY_KEYS, 'decontamination', 'feather', 'protectSubject', 'protection', 'version'].sort();
const MAX_STROKES = 64, MAX_STROKE_POINTS = 512, MAX_CONFIG_JSON = 96 * 1024;
...
function validProtection(protection: any): boolean {
	...
	if (!protection || typeof protection !== 'object' || Object.keys(protection).sort().join() !== 'rect,source,strokes' ||
		typeof protection.source !== 'string' || protection.source.length > 1024 || !Array.isArray(protection.strokes) || protection.strokes.length > MAX_STROKES) return false;
	const { rect } = protection;
	if (rect !== null && (!rect || Object.keys(rect).sort().join() !== 'height,width,x,y' || ![rect.x, rect.y, rect.width, rect.height].every(unit) ||
		rect.x + rect.width > 1.0001 || rect.y + rect.height > 1.0001)) return false;
	return protection.strokes.every((stroke: any) => stroke && Object.keys(stroke).sort().join() === 'label,points,radius' &&
		(stroke.label === 'foreground' || stroke.label === 'background') && unit(stroke.radius) && stroke.radius > 0 && stroke.radius <= 0.25 &&
		Array.isArray(stroke.points) && stroke.points.length >= 2 && stroke.points.length % 2 === 0 && stroke.points.length <= MAX_STROKE_POINTS * 2 && stroke.points.every(unit));
}
...
export function validBannerBlendConfig(config: any): boolean {
	...
	const common = ['multiband', 'poisson', 'fade'].includes(config.method) && typeof config.contentAware === 'boolean' && /^#[0-9a-f]{6}$/i.test(config.surface) &&
	...
	return keys === CURRENT_KEYS.join() && common && Number.isInteger(config.version) && config.version >= 2 && config.version <= 1000 &&
		typeof config.protectSubject === 'boolean' && Number.isInteger(config.feather) && config.feather >= 1 && config.feather <= 12 &&
		unit(config.decontamination) && validProtection(config.protection) && JSON.stringify(config).length <= MAX_CONFIG_JSON;
}
...
export function createBannerBlendRoutes(builder: PathBuilder, database: Database, cardDatabase: CardDatabase): Router {
	...
	app.put(route, async (req: Request<{ id: string }>, res: Response) => {
		...
		if (!validBannerBlendConfig(config) || typeof source !== 'string' || !crop || !images || Object.keys(images).length !== 3) { res.sendStatus(400); return; }
