...
const {
	createRoutes_StorageLocations
} = require('../build/src/routes/storage-locations');
...
test('existing rows get stable typed public IDs and API routes reject row IDs', async () => {
	const directory = fs.mkdtempSync(
		path.join(os.tmpdir(), 'cube-light-public-ids-')
	);
	...
	try {
		...
		assert.equal(
			(await database.getStorageLocation(9)).PublicId,
			locationId
		);
		...
		const createdLocation = await (
			await post('/storage-location', 'New shelf')
		).json();
		assert.match(createdDeck.deckId, /^deck_[A-Za-z0-9_-]{16}$/);
		assert.match(
			createdCollection.collection_id,
			/^collection_[A-Za-z0-9_-]{16}$/
		);
		assert.match(
			createdLocation.storage_location_id,
			/^location_[A-Za-z0-9_-]{16}$/
		);
		...
		assert.equal(
			(await fetch(`${base}/collection/${collectionId}`)).status,
			200
		);
		assert.equal(
			(await fetch(`${base}/storage-location/${locationId}`)).status,
			200
		);
		assert.deepEqual(
			(await (await fetch(`${base}/decks`)).json()).map(
				(deck) => deck.deckId
			),
		...
		assert.deepEqual(
			(
				await (
					await fetch(`${base}/decks?pageStart=1&pageSize=1`)
				).json()
			).map((deck) => deck.deckId),
			[createdDeck.deckId]
		);
		assert.deepEqual(
			(await (await fetch(`${base}/collection/search`)).json()).map(
				(item) => item.collection_id
			),
			[collectionId, createdCollection.collection_id]
		);
		assert.deepEqual(
			(await (await fetch(`${base}/storage-location/search`)).json()).map(
				(item) => item.storage_location_id
			),
			[locationId, createdLocation.storage_location_id]
		);
	} finally {
