...
test('deck history records card UUIDs and copy counts for adds, swaps, and removals', async () => {
	const directory = fs.mkdtempSync(path.join(os.tmpdir(), 'cube-light-history-'));
	...
	try {
		...
		const add = database.applyDeckCardEdit(deckId, [{ uuid: 'card-a', action: 'add', count: 3 }]);
		assert.deepEqual(add.cardsIn, [{ uuid: 'card-a', count: 3 }]);
		assert.deepEqual(add.cardsOut, []);

		assert.equal(database.applyDeckCardEdit(deckId, [{ uuid: 'card-a', action: 'set', count: 3 }]), null);
		...
		database.applyDeckCardEdit(deckId, [{ uuid: 'card-a', action: 'set', count: 0 }]);
		assert.deepEqual((await database.getDeckCards(deckId)).map((args) => {
		...
		assert.deepEqual(updatedHistory[0].details, [{
			field: 'palette', before: null, after: '{"accent":"#123456"}'
		}]);
		assert.deepEqual(updatedHistory[1].details, [{
			field: 'name', before: 'Test deck', after: 'Renamed deck'
		}]);
	} finally {
...
test('history endpoint returns named cards with their UUIDs', async () => {
	const directory = fs.mkdtempSync(path.join(os.tmpdir(), 'cube-light-history-api-'));
	...
	database.applyDeckCardEdit(deckId, [{ uuid: 'card-a', action: 'add', count: 2 }]);
	const cards = { queryCardInfo: async () => [{ uuid: 'card-a', name: 'Example card' }] };
	const app = express();
	app.use(createRoutesDecksId(new PathBuilder('/decks/:id'), database, cards));
	const server = await new Promise((resolve) => {
		const listener = app.listen(0, '127.0.0.1', () => resolve(listener));
	});
	try {
		const port = server.address().port;
		const response = await fetch(`http://127.0.0.1:${port}/decks/${publicId}/history`);
		assert.equal(response.status, 200);
		const body = await response.json();
		assert.equal(body.deckName, 'API deck');
		assert.equal(body.deckId, publicId);
		assert.deepEqual(body.edits[0].cardsIn, [{ uuid: 'card-a', count: 2, name: 'Example card' }]);
		assert.deepEqual(body.edits[0].cardsOut, []);
		assert.equal((await fetch(`http://127.0.0.1:${port}/decks/999/history`)).status, 404);
	} finally {
...
test('banner crops are validated, saved, returned, and recorded in history', async () => {
	const directory = fs.mkdtempSync(path.join(os.tmpdir(), 'cube-light-crop-api-'));
	const database = await Database.Sqlite(path.join(directory, 'app.sqlite'));
	const deckId = String(await database.createDeck('Crop test'));
	const publicId = (await database.getDeck(deckId)).PublicId;
	const app = express();
	app.use(express.json());
	app.use(createRoutesDecksId(new PathBuilder('/decks/:id'), database, {
	...
	try {
		...
		const put = (bannerCrop) => fetch(`${url}/banner-crop`, {
		...
		assert.equal((await put({ ...crop, mobile: { ...crop.mobile, zoom: 8 } })).status, 400);
		assert.equal((await put({ ...crop, mobile: { ...crop.mobile, x: 1.13 } })).status, 400);
		assert.deepEqual((await (await fetch(url)).json()).bannerCrop, crop);
		const putStyle = (topStyle) => fetch(`${url}/top-style`, {
			method: 'PUT', headers: { 'Content-Type': 'application/json' },
		...
		assert.equal((await (await fetch(url)).json()).topStyle, 'full-art');
		const history = await database.getDeckEditHistory(deckId);
		assert.deepEqual(history[0].details, [{ field: 'topStyle', before: 'card', after: 'full-art' }]);
		assert.deepEqual(history[1].details, [{ field: 'bannerCrop', before: null, after: JSON.stringify(crop) }]);
	} finally {
		await new Promise((resolve) => server.close(resolve));
		await database.close();
		fs.rmSync(directory, { recursive: true, force: true });
