...
export type BannerProtectRect = {
	x: number;
	y: number;
	width: number;
	height: number;
};
export type BannerProtectStroke = {
	label: 'foreground' | 'background';
	radius: number;
	points: number[];
};
export type BannerProtection = {
	source: string;
	rect: BannerProtectRect | null;
	strokes: BannerProtectStroke[];
};
...
export type BannerBlend = {
	config: BannerBlendConfig;
	images: Record<BannerBlendVariant, string> | null;
};
export type BannerBlendJob = {
	src: string;
	crop: BannerCrop;
	config: BannerBlendConfig;
};
export type BannerSubjectMask = {
	width: number;
	height: number;
...
export const DEFAULT_PROTECT_RECT: BannerProtectRect = {
	x: 0.15,
	y: 0.05,
	width: 0.7,
	height: 0.9
 ...
 */
export function configForNewArtwork(
	config: BannerBlendConfig,
	src: string,
	mobile: boolean
): BannerBlendConfig {
...
export const BANNER_BLEND_SIZES = {
	desktop: [1440, 224],
	mobile: [720, 224],
	tile: [640, 224]
} as const;
...
export function hasProtection(
	...
	return (
		config.protectSubject &&
		!!src &&
		!!protection &&
		artworkKey(protection.source) === artworkKey(src) &&
		(!!protection.rect ||
			protection.strokes.some((stroke) => stroke.label === 'foreground'))
	);
}
 ...
 */
export function normalizeBannerBlendConfig(
...
export function configForGeneration(
	config: BannerBlendConfig,
	src: string
): BannerBlendConfig {
	const protection =
...
export class BannerWorkCancelled extends Error {
	constructor() {
		super('Replaced by a newer banner request.');
		this.name = 'BannerWorkCancelled';
	}
}
