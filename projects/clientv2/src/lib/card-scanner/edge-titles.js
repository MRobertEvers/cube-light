...
export function trimTitleBand(input) {
	...
	if (!best || hi - lo < 25)
		return { canvas: input, top: 0, left: 0, pad: 0 };
...
export async function tightInkCrops(
	...
	try {
		...
		const regions = [];
		for (const threshold of globalLevels
			...
			for (let i = 0; i < contours.size(); i++) {
				...
				if (
					r.x <= 1 ||
					(!allowTall && r.y <= 1) ||
					r.x + r.width >= src.cols - 2 ||
					(!allowTall && r.y + r.height >= src.rows - 2) ||
					r.height < 8 ||
					r.height > src.rows * (allowTall ? 1 : 0.7) ||
					r.width < 2 ||
					r.width > src.cols * 0.8
				)
					continue;
				parts.push(r);
			}
			for (const seed of parts) {
				const same = parts.filter(
					(p) =>
						Math.abs(
							p.y + p.height / 2 - seed.y - seed.height / 2
						) <
							Math.max(p.height, seed.height) * 0.45 &&
						p.height > seed.height * 0.35 &&
				...
				for (const part of sorted) {
					if (current.length) {
						const last = current.at(-1),
							gap = part.x - last.x - last.width;
						if (
							gap >
							Math.max(
								10,
								Math.max(last.height, part.height) * 0.8
							)
						) {
							groups.push(current);
							current = [];
				...
				for (const same of groups) {
					...
					const r = {
						x: Math.max(0, x - 2),
						y: Math.max(0, y - 2),
						w: Math.min(input.width - x + 2, right - x + 4),
						h: Math.min(input.height - y + 2, bottom - y + 4),
						parts: same.length
					};
					if (
						regions.some(
		...
		return regions
			...
			.map((r) => {
				...
				return { canvas: c, left: r.x, top: r.y, pad: 0 };
			});
	} finally {
		for (const m of [src, gray, binary, contours, hier]) m.delete();
	}
}
...
export function lightTitleBand(input) {
	...
	for (let y = 0; y < h; y++) {
		const v = [];
		for (let x = Math.floor(w * 0.1); x < w * 0.85; x += 2) {
			const i = (y * w + x) * 4;
			v.push(0.299 * p[i] + 0.587 * p[i + 1] + 0.114 * p[i + 2]);
		}
		v.sort((a, b) => a - b);
	...
	let from = 0;
	for (let y = 0; y <= h; y++) {
		if (y < h && rows[y] >= threshold) continue;
		if (y - from >= 10 && y - from < h * 0.85)
			runs.push({ y: from, h: y - from });
		from = y + 1;
	}
	return runs
		.sort((a, b) => b.h - a.h)
		...
		.map((r) => {
			...
			canvas
				.getContext('2d')
				.drawImage(input, 0, r.y, w, r.h, 0, 0, w, r.h);
			return { canvas, left: 0, top: r.y, pad: 0 };
		});
}
...
export async function mserInkCrops(input) {
	const { cv } = await getCV(),
		src = cv.imread(input),
		gray = new cv.Mat(),
		regions = new cv.MatVector(),
		boxes = new cv.RectVector(),
		detector = new cv.MSER();
	try {
		cv.cvtColor(src, gray, cv.COLOR_RGBA2GRAY);
		detector.setDelta(2);
		detector.setMinArea(10);
		detector.setMaxArea(Math.floor(input.width * input.height * 0.4));
		detector.detectRegions(gray, regions, boxes);
		const parts = [];
		for (let i = 0; i < regions.size(); i++) {
			const m = regions.get(i),
				r = cv.boundingRect(m);
			m.delete();
			if (
				r.height < 7 ||
				r.height > input.height * 0.65 ||
				r.width < 2 ||
				r.width > input.width * 0.75
			)
				continue;
			if (
				parts.some(
					(p) =>
						Math.abs(p.x - r.x) < 3 &&
						Math.abs(p.y - r.y) < 3 &&
						Math.abs(p.width - r.width) < 5 &&
						Math.abs(p.height - r.height) < 4
				)
			)
				continue;
			parts.push(r);
		...
		for (const seed of parts) {
				...
				.filter(
					(p) =>
						Math.abs(
							...
							Math.max(p.height, seed.height) * 0.4 &&
						p.height > seed.height * 0.5 &&
						p.height < seed.height * 2
				)
				.sort((a, b) => a.x - b.x);
			let group = [],
				groups = [];
			...
			for (const group of groups) {
				...
				if (
					...
					continue;
				rows.push({ x, y, w, h, count: group.length });
			}
		}
		return rows
			...
			.map((r) => {
				...
				return { canvas, left: r.x - 2, top: r.y - 2, pad: 0 };
			});
	} finally {
		for (const m of [src, gray, regions, boxes, detector]) m.delete();
	}
}
