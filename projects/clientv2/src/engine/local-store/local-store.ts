...
/** The SHA-256 of a value's canonical JSON, as the server computes it. */
export async function hash(crypto: Crypto, value: unknown): Promise<string> {
...
async function checked(tables: Tables, crypto: Crypto, scope: AccountScope, lease?: Lease): Promise<ReplicaMeta> {
    ...
    const meta = await tables.get<ReplicaMeta>('meta', scope.partition) || newMeta(crypto, scope.partition);
...
async function verifyReplica(crypto: Crypto, replica: Replica): Promise<void> {
    const checkpoint = replica.checkpoint;
    if (checkpoint.schemaVersion !== 1 || checkpoint.reducerVersion !== 1 || checkpoint.aggregateId !== replica.id || checkpoint.state.id !== replica.id || await hash(crypto, checkpoint.state) !== checkpoint.stateHash) throw new Error('Invalid checkpoint. Synchronization stopped without discarding local changes.');
    ...
    if (sequence !== replica.sequence || ledgerHash !== replica.hash || await hash(crypto, state) !== await hash(crypto, replica.state)) throw new Error('The downloaded state does not balance.');
}

/** The engine's durable store: aggregates, the outbox, the journal and downloads, over a TableDatabase. */
export class OutboxLocalStore implements LocalStore {
    private readonly driver: TableDatabase;
    private readonly crypto: Crypto;
    constructor(driver: TableDatabase, crypto: Crypto) { this.driver = driver; this.crypto = crypto; }
    ...
    async finishAuth(id: string, session: AuthSession | null, error?: string): Promise<void> {
        await this.driver.transaction(['control', 'meta'], 'readwrite', async (tables) => {
            ...
            if (!error && session) {
                ...
                if (scope) {
                    const meta = await tables.get<ReplicaMeta>('meta', scope.partition) || newMeta(this.crypto, scope.partition);
    ...
    async prepareRefresh(): Promise<AuthCredentials> {
        return this.driver.transaction(['control'], 'readwrite', async (tables) => {
            ...
            credentials.refreshRequestId ||= this.crypto.randomUUID();
    ...
    private async appendIntent(tables: Tables, meta: ReplicaMeta, scope: AccountScope, command: DomainCommand): Promise<Intent> {
        ...
        const intent: Intent = { partition: scope.partition, operationId: this.crypto.randomUUID(), sequence: meta.sequence + 1, command, originalBaseRevision: base?.sequence || 0, dependsOn: previous?.operationId || null, createdAt: new Date().toISOString(), status: 'queued', prepared: null, outcome: null, attempts: 0, nextAttemptAt: 0, error: null };
    ...
    async prepare(lease: Lease): Promise<Intent | null> {
        return this.driver.transaction(['control', 'meta', 'outbox', 'journal'], 'readwrite', async (tables) => {
            ...
            const meta = await checked(tables, this.crypto, scope, lease);
    ...
    async putBlob(scope: AccountScope, data: Blob): Promise<string> {
        ...
        const id = `blob_${await this.crypto.sha256Hex(await data.arrayBuffer())}`;
    ...
    async getBlob(scope: AccountScope, id: string): Promise<LocalBlob | null> {
        return this.driver.transaction(['control', 'meta', 'blobs'], 'readonly', async (tables) => { await checked(tables, this.crypto, scope); return await tables.get<LocalBlob>('blobs', [scope.partition, id]) || null; });
    }
    async uploaded(lease: Lease, id: string, received: number): Promise<void> {
        await this.driver.transaction(['control', 'meta', 'blobs'], 'readwrite', async (tables) => {
            await checked(tables, this.crypto, lease.scope, lease);
    ...
    async rebuild(scope: AccountScope): Promise<LocalNotice> {
        const captured = await this.driver.transaction(['control', 'meta', 'checkpoints', 'events'], 'readonly', async (tables) => ({ meta: await checked(tables, this.crypto, scope), balances: await tables.all<StoredCheckpoint>('checkpoints', scope.partition), events: await tables.all<StoredEvent>('events', scope.partition) }));
        ...
        for (const balance of newest.values()) {
            ...
            await verifyReplica(this.crypto, replica); replicas.push(replica);
        }
        return this.driver.transaction(TABLES, 'readwrite', async (tables) => {
            const meta = await checked(tables, this.crypto, scope);
