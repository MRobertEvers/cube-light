...
export const addCardsSlice = createSlice({
	...
	initialState,
	reducers: {
		openAddCards(state, action: PayloadAction<{ deckId: string }>) {
		...
		setAddCardsText(state, action: PayloadAction<string>) {
...
export const selectAddCards = (state: { addCards: AddCardsState }) =>
	state.addCards;
