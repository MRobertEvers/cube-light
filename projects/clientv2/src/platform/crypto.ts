import type { Crypto } from '../engine/ports';
...
/** The engine's Crypto port, with the fallbacks above for non-secure origins. */
export class WebCrypto implements Crypto {
    randomUUID(): string { return randomUUID(); }
    sha256Hex(data: BufferSource): Promise<string> { return sha256Hex(data); }
}
