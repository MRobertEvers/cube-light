...
export class HttpSyncTransport implements SyncTransport {
    ...
    private async request(path: string, options?: RequestInit, authenticateArg?: boolean): Promise<Response> {
        ...
        try { return await this.transport(path, options, authenticate); }
            ...
            if (!authenticate || !(error instanceof SyncTransportError) || error.status !== 401 || !await this.authStore.credentials()) throw error;
            await this.refresh();
    ...
    private async rotate(): Promise<void> {
        ...
        if (credentials.tokens.refreshExpiresAt <= Date.now()) throw new SyncTransportError('Sign in again to sync your saved edits.', 401);
    ...
    private async transport(path: string, options: RequestInit | undefined, authenticate: boolean): Promise<Response> {
        ...
        try {
            ...
            if (!response.ok) {
                ...
                throw new SyncTransportError(body?.error || `Server request failed (${response.status}).`, response.status, retryAfter);
            }
            return response;
        } catch (error) {
    ...
    async command(request: CommandRequest): Promise<CommandOutcome> {
        ...
        if (result.operationId !== request.operationId || !['accepted', 'conflict', 'rejected'].includes(result.status) || !Array.isArray(result.replicas) || !Array.isArray(result.events)) throw new SyncTransportError('Unsupported command response.', 426);
        return result;
    }
    ...
    async authenticate(type: string, credentials?: { username: string; password: string }): Promise<AuthSession> {
        ...
        if (type === 'login' || type === 'setup') {
            if (!credentials) throw new SyncTransportError('Enter your sign-in details again.', 400);
