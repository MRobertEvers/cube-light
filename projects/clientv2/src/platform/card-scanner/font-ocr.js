...
import { TitleIndexWorkerPool } from '../../workers/title-index/title-index.client';
import {
	findTitleStrips,
	stripCanvas,
	tightInkCrops,
...
/**
 * @param {Array<Array<import('../../workers/title-index/title-index.protocol').ShardMatch>>} shards
 * @param {string[]} names
 */
...
async function createTitleIndex(names, blurArg, fontFileArg, onProgressArg) {
	...
	let pool;
	try {
		pool = await TitleIndexWorkerPool.build(
			names,
			blur,
			new URL(`/ocr/models/fonts/${fontFile}`, location.origin).href,
			onProgress
		);
	} catch (error) {
	...
	return {
		lookup: async function (c) {
			const { v, proj } = descriptor(c);
			return mergeShards(await pool.lookup(v, proj), names);
		},
		dispose: function () {
			pool.dispose();
		}
