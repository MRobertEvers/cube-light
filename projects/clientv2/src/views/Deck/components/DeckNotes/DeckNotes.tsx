...
import type { DeckNote } from '../../../../api/fetch-api-deck';
import {
	fetchAPICreateDeckNote,
	fetchAPIDeleteDeckNote,
	fetchAPIUpdateDeckNote
} from '../../../../api/fetch-api-deck-notes';
...
export function DeckNotes(props: DeckNotesProps) {
	...
	async function create() {
		...
		if (
			await run(
				'new',
				() => fetchAPICreateDeckNote(deckId, text),
	...
	async function saveEdit() {
		...
		if (
			await run(
				noteId,
				() => fetchAPIUpdateDeckNote(deckId, noteId, text),
				'Unable to save the note. Please try again.'
			)
		)
			setEditing(null);
	...
	function remove(note: DeckNote) {
		...
		void run(
			note.noteId,
			() => fetchAPIDeleteDeckNote(deckId, note.noteId),
