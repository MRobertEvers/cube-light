...
export function AddCard(props: AddCardProps) {
	...
	useEffect(() => {
		if (viewIsDropDownVisible && activeIndex >= 0) {
			(suggestionsRef.current?.children[activeIndex] as HTMLElement | undefined)?.scrollIntoView({ block: 'nearest' });
		}
	}, [activeIndex, viewIsDropDownVisible]);

	const workerResponseHandler = useMemo(() => {
		return createResponseHandler((builder) => {
			builder.addCase(DeckWorkerMessages.getSuggestions, async (response) => {
				const { query, requestId, sorted, set, error: searchError } = response.payload;
				if (requestId !== requestIdRef.current || query !== queryRef.current) return;
				await waitForMinimumStatusDuration(searchStartedAt.current);
				if (requestId !== requestIdRef.current || query !== queryRef.current) return;
				dispatch(Actions.setSuggestionsData({ sorted, set }));
				setIsSearching(false);
				if (searchError) setError('Unable to search cards. Please try again.');
				setActiveIndex(-1);
				dispatch(Actions.setViewIsDropDownVisible(
					sorted.length > 0 && document.activeElement === addItemInputRef.current
				));
			});
	...
	const exactMatch = suggestions.sorted.find(
		(suggestion) => suggestion.toLowerCase() === viewAddItemText.trim().toLowerCase()
	);
	const resolvedCardName = !isSearching && (exactMatch || suggestions.sorted.length === 1)
		? exactMatch ?? suggestions.sorted[0]
		: null;
	const canSubmit = Boolean(resolvedCardName) && !isSubmitting;
	const showSuggestions = viewIsDropDownVisible && suggestions.sorted.length > 0;

	function selectSuggestion(suggestion: string, keepFocus = true) {
		requestIdRef.current += 1;
		queryRef.current = suggestion;
		dispatch(Actions.setViewAddItemText(suggestion));
		dispatch(Actions.setSuggestionsData({ sorted: [suggestion], set: new Set([suggestion.toLowerCase()]) }));
	...
	function submitCard() {
		...
		setError(null);
		dispatch(Actions.setViewIsDropDownVisible(false));
		postToWorker(DeckWorkerMessages.addCard({
			deckId,
		...
		>
			...
			<div className={styles['field']}>
				...
				<div className={styles['search-field']}>
					<input
						...
						onFocus={() => {
							if (suggestions.sorted.length > 0) dispatch(Actions.setViewIsDropDownVisible(true));
						}}
						onBlur={() => dispatch(Actions.setViewIsDropDownVisible(false))}
						onChange={(event) => {
							...
							if (query.trim()) {
								...
								postToWorker(DeckWorkerMessages.getSuggestions({ query, requestId: requestIdRef.current }));
							}
						}}
						onKeyDown={(event) => {
							if ((event.key === 'ArrowDown' || event.key === 'ArrowUp') && suggestions.sorted.length > 0) {
								event.preventDefault();
								dispatch(Actions.setViewIsDropDownVisible(true));
								setActiveIndex((current) => {
									if (event.key === 'ArrowDown') return (current + 1) % suggestions.sorted.length;
									return current <= 0 ? suggestions.sorted.length - 1 : current - 1;
								});
							} else if (event.key === 'Enter' && showSuggestions && activeIndex >= 0) {
								event.preventDefault();
								selectSuggestion(suggestions.sorted[activeIndex]);
							} else if (event.key === 'Enter' && showSuggestions && suggestions.sorted.length === 1 && !exactMatch) {
								event.preventDefault();
								selectSuggestion(suggestions.sorted[0]);
							} else if (event.key === 'Tab' && !event.shiftKey && showSuggestions && suggestions.sorted.length === 1) {
								selectSuggestion(suggestions.sorted[0], false);
							}
						}}
					/>
					<div className={styles['search-indicator']} aria-hidden="true">
						{isSearching ? <Spinner /> : viewAddItemText ? (
							resolvedCardName ? <EnterIcon /> : <AlertIcon />
						) : null}
					</div>
					{showSuggestions && (
						<ul ref={suggestionsRef} id={listId} className={styles['suggestions']} role="listbox" aria-label="Card suggestions">
							{suggestions.sorted.map((suggestion, index) => (
								<li
									...
									className={index === activeIndex ? styles['active-suggestion'] : undefined}
				...
				<p id={hintId} className={styles['field-hint']}>
					{isSearching ? 'Searching cards…' : viewAddItemText && !resolvedCardName
			...
			<div className={styles['quantity-row']}>
				<div>
					...
					<span className={styles['quantity-subtitle']}>Number of copies</span>
				</div>
				<Counter
					count={viewAddItemCount}
					setCount={(count) => dispatch(Actions.setViewAddItemCount(count))}
				/>
			</div>
			<div className={styles['modal-buttons']}>
				<Button className={styles['cancel-button']} disabled={isSubmitting} onClick={() => onEvent({ type: AddCardEventType.CLOSE })}>
					Cancel
				</Button>
				<Button className={styles['add-button']} type="submit" disabled={!canSubmit}>
					{isSubmitting ? <><Spinner /> Adding…</> : 'Add card'}
				</Button>
			</div>
		</form>
	);
