...
import { loadDeck, selectDeck, selectDeckError } from '../../store/decks/decks.state';
...
import { appearanceActions, appearanceView, selectAppearanceSettings } from '../../store/appearance-settings/appearance-settings.state';
import { BannerCardPickerModal } from './BannerCardPickerModal';
import { BannerCardSection, BlendSection, CropSection, PaletteSection, TopStyleSection } from './DeckSettingsSections';
...
export function DeckSettingsPage(props: DeckSettingsPageProps) {
	...
	const loadError = useSelector((root: Parameters<typeof selectDeckError>[0]) => selectDeckError(root, deckId));
	...
	if (!data || showInitialLoading) {
		return <Page><main className={styles['loading']}>{!showInitialLoading && loadError ? 'Unable to load deck settings.' : <LoadingIndicator />}</main></Page>;
	}

	return <Page>
		<BannerCardPickerModal deckId={deckId} deckName={data.name} onSaved={() => dispatch(appearanceActions.bannerSaved(deckId))} />
		<main className={styles['page']} style={paletteStyle}>
			<div className={styles['container']}>
				<Link className={styles['back-link']} to={`/deck/${deckId}`}>← Back to deck</Link>
				<header className={styles['heading']}><h1>Deck appearance</h1><p>{data.name}</p></header>
				<div className={styles['layout']}>
					<BannerCardSection deckId={deckId} data={data} view={view} />
					<TopStyleSection deckId={deckId} data={data} view={view} />
					<CropSection deckId={deckId} data={data} view={view} />
					<BlendSection deckId={deckId} data={data} view={view} />
					<PaletteSection deckId={deckId} data={data} view={view} />
				</div>
			</div>
		</main>
	</Page>;
