...
import { ImageScanContent, statusText, totalAdded } from 'src/components/ImageCardImport/DeckImageScanCard';
...
export function DeckImageScanPage(props: { deckId: string; taskId: string }) {
	...
	const task = useImageImportQueue().find((item) => item.id === taskId && item.deckId === deckId);

	// Scans live in memory, so a reload or stale link has nothing to show.
	if (!task) return <Navigate to={`/deck/${deckId}`} replace />;

	const actions = <div className={styles.pageActions}>
		<button type="button" className={styles.primary} onClick={() => navigate(`/deck/${deckId}`)}>
