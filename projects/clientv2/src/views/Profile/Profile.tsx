...
import { fetchAPICardNames } from '../../api/fetch-api-card-names';
import {
	CardPrinting,
...
export function Profile() {
	...
	async function loadPrintings(name: string, preferredUuid?: string | null) {
		...
		try {
			const items = (await fetchAPICardPrintings(name)).filter(
				(item) => !!item.art
			);
	...
	async function save() {
		...
		setSavedMessage('');
		try {
			const nextUser = await fetchAPISetProfile(draft);
			updateUser(nextUser);
