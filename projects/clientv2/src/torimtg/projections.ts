import type { DeckState, EventEnvelope, Query, WorkState } from '@torimtg/core';
import type { Dataset } from './types';

export function blobUrl(id: string): string { return `/__tori_blob/${id}`; }

function art(deck: DeckState, catalog: Record<string, any>): string | null {
    return deck.art || catalog[deck.bannerCardUuid || '']?.art || catalog[Object.keys(deck.cards)[0]]?.art || null;
}
...
export function projectQuery(data: Dataset, query: Query): unknown {
    const states = data.states.filter((state) => !state.deleted);
    const catalog = data.catalog as Record<string, any>;
    switch (query.type) {
        case 'decks': return states.filter((state): state is DeckState => state.kind === 'deck').map((deck) => ({ deckId: deck.id, name: deck.name, art: art(deck, catalog), bannerBlend: blend(deck), createdAt: deck.createdAt, updatedAt: deck.updatedAt }));
        case 'deck': {
            const deck = states.find((state) => state.id === query.id);
            if (deck?.kind !== 'deck') return null;
            const banner = catalog[deck.bannerCardUuid || ''];
            return { name: deck.name, icon: art(deck, catalog), bannerCardUuid: deck.bannerCardUuid, bannerCard: banner ? { name: banner.name, uuid: banner.uuid, setCode: banner.setCode, art: banner.art } : null, palette: deck.palette, bannerCrop: deck.bannerCrop, bannerBlend: blend(deck), topStyle: deck.topStyle, lastEdit: deck.updatedAt, cards: Object.entries(deck.cards).map((entry) => {
                const [uuid, count] = entry;
                return { name: uuid, types: 'Unknown', manaCost: '', image: '', art: '', setCode: '', text: '', legalities: {}, ...catalog[uuid], uuid, count };
            }) };
        }
        ...
        case 'history': {
            const deck = states.find((state) => state.id === query.id);
            return { deckId: query.id, deckName: deck?.kind === 'deck' ? deck.name : '', edits: [...data.events.filter((event) => event.aggregateId === query.id).sort((a, b) => b.aggregateSequence - a.aggregateSequence).map((event) => historyEntry(event, catalog)).filter(Boolean), ...(data.legacyHistory || [])] };
        }
        case 'resource': return null;
    }
}

function historyEntry(envelope: EventEnvelope, catalog: Record<string, any>): unknown {
    ...
    if (event.type === 'CardQuantitiesAdjusted') {
        for (const change of event.changes) (change.delta > 0 ? entry.cardsIn : entry.cardsOut).push({ uuid: change.uuid, name: catalog[change.uuid]?.name || null, count: Math.abs(change.delta) });
