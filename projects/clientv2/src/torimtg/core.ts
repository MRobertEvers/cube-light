...
export function createToriMTG(store: LocalStore, worker: ServiceWorkerApi): ToriMTG {
    ...
    const channel = typeof BroadcastChannel !== 'undefined' ? new BroadcastChannel('torimtg-commits') : null;
    let started = false;
    function emit(notice: LocalNotice) { for (const listener of listeners) listener(notice); }
    worker.subscribe(emit);
    if (channel) channel.onmessage = function (event) { emit(event.data); };
    function committed(notice: LocalNotice) { emit(notice); channel?.postMessage(notice); }
    function wake() { void worker.wake().catch(() => undefined); }
    ...
    async function open(): Promise<void> {
        ...
        if (!started && typeof window !== 'undefined') {
            started = true;
            for (const name of ['online', 'focus']) window.addEventListener(name, wake);
            document.addEventListener('visibilitychange', () => { if (!document.hidden) wake(); });
            setInterval(() => { if (!document.hidden) wake(); }, 15000);
    ...
    async function read<T>(query: Query): Promise<LocalSnapshot<T>> {
        ...
        for (const resource of data.resources) {
            ...
            if (descriptor.type === 'history' && descriptor.id === query.id) {
                ...
                const byId = new Map([...archived, ...data.events].map((event) => [event.eventId, event]));
                data.events = [...byId.values()];
            }
        }
        const pending = data.intents.filter(outstanding).filter((intent) => !('id' in query) || intent.command.id === query.id);
        let value: unknown;
        ...
        if (current.partition !== active.partition || current.generation !== active.generation) throw new Error('The active account changed.');
        return { data: value as T | null, presence, localRevision: data.meta.revision, lastValidatedAt: data.meta.validatedAt, pendingCount: pending.length, conflictCount: pending.filter((intent) => ['conflict', 'rejected'].includes(intent.status)).length, refresh: data.meta.refresh, lastError: data.meta.error };
    }
    async function requestRefresh(query?: Query): Promise<void> {
        const resource = query?.type === 'resource' ? query.resource : query?.type === 'history' ? { type: 'history' as const, id: query.id } : undefined;
        await store.refresh(await scope(), resource, query === undefined); wake();
    }
    async function execute(command: DomainCommand): Promise<LocalCommit> {
        const commit = await store.commit(await scope(), command);
        committed(commit); wake(); return commit;
    }
    async function resolve(operationId: string, choice: 'server' | 'mine'): Promise<void> {
        committed(await store.resolve(await scope(), operationId, choice)); wake();
    }
    async function session(): Promise<Session> {
        ...
        if (auth.session?.user && !auth.locked) {
            ...
            wake(); return auth.session;
        }
        if (auth.locked && auth.session) return { ...auth.session, user: null, setupRequired: false };
        const id = await store.startAuth('session');
        await worker.authenticate(id);
        const updated = await store.auth();
        if (!updated.session) throw new Error(updated.error || 'Could not reach the server.');
        return updated.session;
    }
    async function signIn(username: string, password: string, setup: boolean): Promise<PublicUser> {
        await open();
        const id = await store.startAuth(setup ? 'setup' : 'login');
        await worker.authenticate(id, { username, password });
        const auth = await store.auth();
        if (!auth.session?.user || auth.locked) throw new Error(auth.error || 'Could not sign in.');
        committed({ partition: `${auth.session.serverInstanceId}:${auth.session.user.id}:shared`, generation: auth.generation, localRevision: 0 });
        wake(); return auth.session.user;
    }
    async function signOut(): Promise<void> {
        const id = await store.startAuth('logout');
        committed({ partition: '', generation: (await store.auth()).generation, localRevision: 0 });
    ...
    async function saveBlob(blob: Blob): Promise<string> { return store.putBlob(await scope(), blob); }
    async function blob(id: string): Promise<Blob> {
        const saved = await store.getBlob(await scope(), id);
        if (saved) return saved.data;
        const query: Query = { type: 'resource', resource: { type: 'blob', id } };
    ...
    async function pending(): Promise<Intent[]> { return (await store.dataset(await scope())).intents.filter(outstanding); }
    async function exportPending(): Promise<Blob> {
        const active = await scope();
        const data = await store.dataset(active);
        const intents = data.intents.filter(outstanding);
        const ids = new Set(intents.flatMap((intent) => intent.command.type === 'work.queue' ? [intent.command.blobId] : intent.command.type === 'deck.blend' ? Object.values(intent.command.blend.images) : []));
        const blobs = [];
        for (const id of ids) {
            ...
            blobs.push({ id, contentType: local.data.type, base64: btoa(binary) });
        }
        return new Blob([JSON.stringify({ version: 1, partition: active.partition, exportedAt: new Date().toISOString(), intents, states: data.states, blobs }, null, 2)], { type: 'application/json' });
    }
