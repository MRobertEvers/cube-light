...
export function readableAccent(accent: string, background: string): string {
	...
	for (let amount = 0.05; amount <= 1.001; amount += 0.05) {
		const candidate = `#${channels.map((channel) => Math.round(channel * (1 - amount) + target * amount).toString(16).padStart(2, '0')).join('')}`;
...
function extractPalette(image: HTMLImageElement): CardPalette | null {
	...
	try {
		...
		const buckets = Array.from({ length: HUE_BUCKETS }, () => ({ weight: 0, count: 0, x: 0, y: 0, saturation: 0, lightness: 0 }));
		for (let index = 0; index < pixels.length; index += 4) {
			if (pixels[index + 3] < 128) continue;
			const { hue, saturation, lightness } = rgbToHsl(pixels[index], pixels[index + 1], pixels[index + 2]);
			if (saturation < 0.16 || lightness < 0.1 || lightness > 0.9) continue;
		...
		const windowWeight = (index: number) => [-1, 0, 1].reduce(
			(sum, offset) => sum + buckets[(index + offset + HUE_BUCKETS) % HUE_BUCKETS].weight, 0
		);
		const windowCount = (index: number) => [-1, 0, 1].reduce(
			(sum, offset) => sum + buckets[(index + offset + HUE_BUCKETS) % HUE_BUCKETS].count, 0
		);
		let best = -1;
		let bestWeight = 0;
		...
		const summarize = (index: number) => {
			const neighbors = [-1, 0, 1].map((offset) => buckets[(index + offset + HUE_BUCKETS) % HUE_BUCKETS]);
			const totals = neighbors.reduce((sum, bucket) => ({
			...
			return {
				hue: (Math.atan2(totals.y, totals.x) * 180 / Math.PI + 360) % 360,
		...
		const distanceFromDominant = (index: number) =>
			Math.min(Math.abs(index - best), HUE_BUCKETS - Math.abs(index - best)) * 360 / HUE_BUCKETS;
		...
		for (let index = 0; index < HUE_BUCKETS; index++) {
			const weight = windowWeight(index);
			if (distanceFromDominant(index) < 80 || weight < bestWeight * 0.12 ||
				windowCount(index) < SAMPLE_SIZE * SAMPLE_SIZE * 0.025 ||
				summarize(index).saturation < 0.26) continue;
		...
		if (accentBucket === best) {
			...
			for (let index = 0; index < HUE_BUCKETS; index++) {
				const weight = windowWeight(index);
				if (distanceFromDominant(index) < 50 || weight < bestWeight * 0.1 ||
					windowCount(index) < SAMPLE_SIZE * SAMPLE_SIZE * 0.025) continue;
		...
		const saturation = Math.min(0.75, Math.max(0.45, accent.saturation * 1.3));
		let accentLightness = Math.min(0.42, Math.max(0.32, 0.35 + (accent.lightness - 0.4) * 0.15));
		while (accentContrast(hue, saturation, accentLightness) < 5.6) accentLightness -= 0.01;

		return {
			accent: hslColor(hue, saturation, accentLightness),
			surface: hslColor(surface.hue, Math.min(0.5, Math.max(0.22, surface.saturation * 0.9)), 0.87),
			wash: hslColor(surface.hue, Math.min(0.32, Math.max(0.14, surface.saturation * 0.65)), 0.95),
			border: hslColor(surface.hue, Math.min(0.45, Math.max(0.18, surface.saturation * 0.8)), 0.76)
		};
	} catch {
...
export function useCardPalette(imageUrl: string | null | undefined): CardPalette | null {
	const [result, setResult] = useState<{ url: string; palette: CardPalette | null } | null>(null);

	useEffect(() => {
		if (!imageUrl) return;
		if (paletteCache.has(imageUrl)) {
			setResult({ url: imageUrl, palette: paletteCache.get(imageUrl) ?? null });
			return;
		}
