...
import type {
	BannerWorkerRequest,
	BannerWorkerResponse
} from './banner-blend.worker';
...
export class BannerBlendCancelled extends Error {
	constructor() {
		super('Replaced by a newer banner request.');
		this.name = 'BannerBlendCancelled';
	}
}

type Running = { cancel: () => void };
type WorkerRequest = BannerWorkerRequest extends infer R
	? R extends BannerWorkerRequest
		? Omit<R, 'id'>
		: never
	: never;
...
function runWorker<T extends BannerWorkerResponse>(
	slot: Slot,
	...
	return new Promise<T>((resolve, reject) => {
		...
		const timeout = window.setTimeout(() => {
			...
			reject(
				new Error(
					'Banner rendering timed out. Try a smaller subject selection or try again.'
				)
			);
		}, timeoutMs);
		...
		worker.onerror = () => {
			...
			reject(
				new Error(
					'Banner rendering is unavailable in this browser. Try a current version of Chrome, Edge, Firefox, or Safari.'
				)
			);
		};
...
export async function generateAndSaveBannerBlend(
	...
	>(
		'generate',
		{ kind: 'generate', job },
		(progress) => onProgress?.(progress.message, progress.fraction),
		120000
	);
	...
	const response = await apiFetch(`${API_URI}/decks/${deckId}/banner-blend`, {
		...
		body: JSON.stringify({
			...
			crop: job.crop,
			config: job.config,
			images: result.images satisfies Record<BannerBlendVariant, string>
		})
	});
	if (!response.ok)
		throw new Error(
			response.status === 409
				? 'The artwork or crop changed while rendering. Please generate again.'
				: response.status === 400
					? 'The server rejected the banner settings. Reset the subject selection and try again.'
 ...
 */
export async function previewBannerBlend(
	src: string,
	crop: BannerCrop,
	config: BannerBlendConfig
): Promise<Record<BannerBlendVariant, string>> {
	const job: BannerBlendJob = {
		src,
		crop,
		config: configForGeneration(config, src)
	};
	const result = await runWorker<
		Extract<BannerWorkerResponse, { images: unknown }>
	...
	return {
		...
		tile: url(result.images.tile)
	};
}

export function cancelBannerBlendPreview(): void {
	running.preview?.cancel();
}
...
export async function previewSubjectMask(
	...
	const result = await runWorker<
		Extract<BannerWorkerResponse, { mask: unknown }>
	>('mask', { kind: 'mask', src, protection, feather }, onProgress, 60000);
	return { ...result.mask, milliseconds: result.timings.total };
}

export function cancelSubjectMaskPreview(): void {
	running.mask?.cancel();
}
