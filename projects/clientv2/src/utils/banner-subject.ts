...
import type { BannerWasm, SubjectLayer, SubjectStage } from './banner-wasm';
...
export function protectionLabels(protection: BannerProtection, width: number, height: number): Uint8Array {
	...
	for (const stroke of protection.strokes) {
		...
		for (let i = 0; i + 1 < points.length; i += 2) {
			...
			const steps = Math.max(1, Math.ceil(Math.sqrt((nx - x) * (nx - x) + (ny - y) * (ny - y)) / Math.max(0.5, radius / 2)));
...
export type SubjectOptions = { feather: number; decontamination: number; onStage?: (stage: SubjectStage) => void };
 ...
 */
export function computeSubjectLayer(wasm: BannerWasm, rgba: Uint8ClampedArray, width: number, height: number, protection: BannerProtection, options: SubjectOptions): SubjectLayer {
	...
	try {
		return wasm.subject(rgba, width, height, workLabels, workWidth, workHeight, fullLabels, GRABCUT_ITERATIONS, options.feather, options.decontamination);
	} finally { wasm.onStage = null; }
}
