...
function tileStarts(length: number, size: number, step: number): number[] {
	...
	while (true) {
		...
		if (next + size >= length) {
			...
			if (final - starts[starts.length - 1] < step / 4)
				starts[starts.length - 1] = final;
...
function getRegions(width: number, height: number): ImageRegion[] {
	...
	return ys.flatMap((y) =>
		xs.map((x) => ({
			x,
			y,
			width: xs.length === 1 ? width : Math.min(1000, width - x),
...
function createMatcher(names: string[]) {
	const worker = new Worker(
		new URL('./card-name-match.worker.ts', import.meta.url),
		{ type: 'module' }
	);
	...
	const pending = new Map<
		number,
		{
			resolve: (candidates: CardImageCandidate[]) => void;
