...
export type BannerWorkerRequest =
	...
	| { kind: 'mask'; id: number; src: string; protection: BannerProtection; feather: number };
export type BannerWorkerResponse =
	| { id: number; progress: { message: string; fraction: number } }
	| { id: number; images: Record<BannerBlendVariant, string>; timings: Record<string, number> }
	| { id: number; mask: { width: number; height: number; alpha: Uint8ClampedArray }; timings: Record<string, number> }
...
async function decode(src: string): Promise<ImageBitmap> {
	...
	try { response = await fetch(src); } catch { throw new Error('Unable to download the banner artwork. Check your connection and try again.'); }
	if (!response.ok) throw new Error(`Unable to load the banner artwork (HTTP ${response.status}). Choose the artwork again or retry.`);
	try { return await createImageBitmap(await response.blob()); } catch { throw new Error('The banner artwork could not be decoded. Choose a different printing.'); }
}
...
function placement(variant: BannerBlendVariant, crop: BannerCrop, bitmap: { width: number; height: number }) {
...
function frameSubject(layer: SubjectLayer, place: ReturnType<typeof placement>): SubjectFrame {
	const { width, height, artWidth, left, top, imageWidth, imageHeight } = place;
	const n = width * height, alpha = new Float32Array(n), foregroundDelta = new Float32Array(n * 3), backgroundDelta = new Float32Array(n * 3);
	const sw = layer.width, sh = layer.height;
	for (let y = 0; y < height; y++) {
		const v = (y + 0.5 - top) / imageHeight * sh - 0.5;
		if (v < -0.5 || v > sh - 0.5) continue;
		const y0 = Math.max(0, Math.floor(v)), y1 = Math.min(sh - 1, y0 + 1), fy = Math.min(1, Math.max(0, v - y0));
		for (let x = 0; x < Math.floor(artWidth); x++) {
			const u = (x + 0.5 - left) / imageWidth * sw - 0.5;
			if (u < -0.5 || u > sw - 0.5) continue;
			const x0 = Math.max(0, Math.floor(u)), x1 = Math.min(sw - 1, x0 + 1), fx = Math.min(1, Math.max(0, u - x0));
			const w00 = (1 - fx) * (1 - fy), w10 = fx * (1 - fy), w01 = (1 - fx) * fy, w11 = fx * fy;
			const a = y0 * sw + x0, b = y0 * sw + x1, c = y1 * sw + x0, d = y1 * sw + x1, p = y * width + x;
			alpha[p] = layer.alpha[a] * w00 + layer.alpha[b] * w10 + layer.alpha[c] * w01 + layer.alpha[d] * w11;
			for (let ch = 0; ch < 3; ch++) {
				foregroundDelta[p * 3 + ch] = layer.foregroundDelta[a * 3 + ch] * w00 + layer.foregroundDelta[b * 3 + ch] * w10 + layer.foregroundDelta[c * 3 + ch] * w01 + layer.foregroundDelta[d * 3 + ch] * w11;
				backgroundDelta[p * 3 + ch] = layer.backgroundDelta[a * 3 + ch] * w00 + layer.backgroundDelta[b * 3 + ch] * w10 + layer.backgroundDelta[c * 3 + ch] * w01 + layer.backgroundDelta[d * 3 + ch] * w11;
			}
		}
	}
...
async function generate(id: number, job: BannerBlendJob) {
	...
	const progress = (message: string, fraction: number) => post({ id, progress: { message, fraction } });
	...
	try {
		...
		if (hasProtection(config, job.src)) {
			...
			layer = computeSubjectLayer(pixels.data, pixels.width, pixels.height, config.protection!, {
				...
				onStage: (stage) => {
					...
					else if (stage === 'refine') { lap('segment'); progress('Refining subject edges…', 0.28); }
					else { lap('refine'); progress('Removing background color from subject edges…', 0.34); }
				}
			});
			lap('decontaminate');
		}
		...
		for (const [index, variant] of variants.entries()) {
			const start = layer ? 0.4 : 0.1, fraction = start + (0.95 - start) * index / variants.length;
...
async function mask(id: number, src: string, protection: BannerProtection, feather: number) {
	...
	try {
		...
		for (let p = 0; p < alpha.length; p++) alpha[p] = Math.round(layer.alpha[p] * 255);
		post({ id, mask: { width: layer.width, height: layer.height, alpha }, timings: { total: Math.round(performance.now() - start) } }, [alpha.buffer]);
	} finally { bitmap.close(); }
}

self.addEventListener('message', async (event: MessageEvent<BannerWorkerRequest>) => {
	const request = event.data;
	try {
		if (request.kind === 'generate') await generate(request.id, request.job);
		else await mask(request.id, request.src, request.protection, request.feather);
	} catch (error) {
		post({ id: request.id, error: error instanceof Error ? error.message : 'Unable to render the banner.' });
	}
});
