...
import { readAllCardNames } from 'src/redux/cards/cards.thunks';
import { addScanCandidate, dismissScan } from 'src/redux/scans/scans.thunks';
...
import { workProgress, workStatusText } from 'src/ui/kit/utils/work-status';
...
export function ImageScanContent(props: { task: ImageScanTask }) {
	...
	useEffect(() => {
		let active = true;
		void dispatch(readAllCardNames())
			.then((result) => {
	...
	async function add(name: string, count: number): Promise<boolean> {
		...
		setError(null);
		try {
			await dispatch(addScanCandidate(task.id, name, count));
			return true;
		} catch (cause) {
			setError(
