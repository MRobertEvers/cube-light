...
export function ImageCardImport(props: Props) {
	...
	async function start() {
		...
		try {
			...
			if (!targetDeckId) {
				targetDeckId = await dispatch(createDeck(deckName.trim()));
				setCreatedDeckId(targetDeckId);
			}
			if (deferToDesktop) {
				await dispatch(queueForDesktop(targetDeckId, file, pipeline));
				onComplete(targetDeckId);
				return;
			}
			const taskId = dispatch(scanPhoto(targetDeckId, file, pipeline));
