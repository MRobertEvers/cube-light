...
export type DeckHistoryCard = { uuid: string; name: string | null; count: number };
...
export async function fetchAPIDeckHistory(deckId: string): Promise<DeckHistoryResponse> {
	const response = await fetch(`${API_URI}/decks/${deckId}/history`);
	if (!response.ok) throw new Error(`Could not load deck history (${response.status})`);
