...
export function ImageCardImport(props: Props) {
	...
	const start = async () => {
		if (!file || isStarting || (mode === 'create' && !deckName.trim())) return;
		setIsStarting(true);
		setError(null);
		try {
	...
	return createPortal(
		<div className={styles.backdrop}>
			<section className={styles.panel} role="dialog" aria-modal="true" aria-labelledby="image-import-title">
				...
				<div className={styles.body}>
					<div className={styles.controls}>
						{mode === 'create' && <label>Deck name
							<input autoFocus value={deckName} onChange={(event) => setDeckName(event.target.value)} maxLength={1024} placeholder="Give your deck a name" disabled={isStarting} />
						</label>}
						<label>Card photo
							<input type="file" accept="image/*" disabled={isStarting} onChange={(event) => {
					...
					{previewUrl && <div className={styles.preview}>
						<img src={previewUrl} alt="Card photo selected for background scan" />
					</div>}
					{error && <p className={styles.error} role="alert">{error}</p>}
					{createdDeckId && error && <p>The deck was created. Retry scanning or open the deck.</p>}
				</div>
				<footer className={styles.footer}>
					{createdDeckId && error && <button type="button" onClick={() => onComplete(createdDeckId)}>Open deck</button>}
					<button type="button" onClick={onClose} disabled={isStarting}>Cancel</button>
					<button type="button" className={styles.primary} onClick={() => { void start(); }} disabled={!file || isStarting || (mode === 'create' && !deckName.trim())}>
						{isStarting ? 'Starting…' : mode === 'create' ? 'Create deck and scan' : 'Start image scan'}
					</button>
				</footer>
			</section>
		</div>,
