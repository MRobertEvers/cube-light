...
export function DeckBannerCard(props: Props) {
	...
	return <div className={`${styles.card} ${styles[variant]}`}>
		<div className={styles.art}>
			<div className={styles.desktopArt}>
				<BannerArtwork src={src} frame={crop.desktop} onChange={onCropChange ? (frame) => onCropChange('desktop', frame) : undefined} label={onCropChange ? 'Drag desktop banner art to reposition' : undefined} />
			</div>
			<div className={styles.mobileArt}>
				<BannerArtwork src={src} frame={crop.mobile} onChange={onCropChange ? (frame) => onCropChange('mobile', frame) : undefined} label={onCropChange ? 'Drag mobile banner art to reposition' : undefined} />
			</div>
		</div>
		<div className={styles.info}>
			...
			<span className={styles.subtitle}>Updated <strong>{toFriendlyDate(updatedAt)}</strong></span>
		</div>
	</div>;
}
