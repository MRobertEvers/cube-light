...
static double
clampd(double n, double lo, double hi)
{
    return n < lo ? lo : n > hi ? hi : n;
}
static int
clampi(int n, int lo, int hi)
{
    return n < lo ? lo : n > hi ? hi : n;
}
static double
smooth(double lo, double hi, double n)
{
    double t = clampd((n - lo) / (hi - lo), 0, 1);
    return t * t * (3 - 2 * t);
}
static double
linear_from_srgb(double n)
{
    n /= 255;
    return n <= 0.04045 ? n / 12.92 : pow((n + 0.055) / 1.055, 2.4);
}
static int
srgb_from_linear(double n)
{
    n = clampd(n, 0, 1);
...
init_linear(void)
{
    if( linear_ready )
        return;
    for( int i = 0; i < 256; i++ )
        LINEAR[i] = (float)linear_from_srgb(i);
    linear_ready = 1;
}
...
static int
graph_init(Graph* g, int nodes, int edges)
{
    memset(g, 0, sizeof *g);
    g->nodes = nodes;
    g->capacity = edges * 2;
    g->first = ALLOC(int, nodes);
    g->head = ALLOC(int, g->capacity);
    g->next = ALLOC(int, g->capacity);
    g->cap = ALLOC(double, g->capacity);
    g->terminal = ALLOC(double, nodes);
    g->parent = ALLOC(int, nodes);
    g->stamp = ALLOC(int, nodes);
    g->distance = ALLOC(int, nodes);
    g->sink = ALLOC(uint8_t, nodes);
    g->queued = ALLOC(uint8_t, nodes);
    if( !g->first || !g->head || !g->next || !g->cap || !g->terminal || !g->parent || !g->stamp ||
        !g->distance || !g->sink || !g->queued )
        return 0;
...
static void
graph_tweights(Graph* g, int node, double from_source, double to_sink)
{
...
static void
graph_edge(Graph* g, int i, int j, double forward, double backward)
{
    int a = g->arcs;
    g->head[a] = j;
    g->cap[a] = forward;
    ...
    g->head[a + 1] = i;
    g->cap[a + 1] = backward;
    g->next[a + 1] = g->first[j];
    g->first[j] = a + 1;
    g->arcs += 2;
}
/* Free nodes count as source, as in OpenCV. */
static int
graph_in_source(const Graph* g, int node)
{
    return g->parent[node] == MF_NONE || !g->sink[node];
}
...
static int
list_push(IntList* list, int value)
{
...
static int
graph_maxflow(Graph* g)
{
    int n = g->nodes, qsize = n + 1;
    int *first = g->first, *head = g->head, *next = g->next, *parent = g->parent, *stamp = g->stamp,
        *distance = g->distance;
...
#define SET_ACTIVE(i)                                                                              \
    ...
    {                                                                                              \
        ...
        {                                                                                          \
            queued[_i] = 1;                                                                        \
            active[active_tail] = _i;                                                              \
            active_tail = (active_tail + 1) % qsize;                                               \
            active_count++;                                                                        \
        }                                                                                          \
    } while( 0 )
#define ORPHAN(i)                                                                                  \
    ...
    while( ok )
    {
        ...
        if( i == MF_NONE )
        {
            ...
            {
                int k = active[active_head];
                active_head = (active_head + 1) % qsize;
                active_count--;
                queued[k] = 0;
                if( parent[k] != MF_NONE )
                {
        ...
        if( !sink[i] )
        {
            for( int a = first[i]; a != MF_NONE; a = next[a] )
            {
                ...
                else if( stamp[j] <= stamp[i] && distance[j] > distance[i] )
                {
                    parent[j] = a ^ 1;
                    stamp[j] = stamp[i];
                    distance[j] = distance[i] + 1;
        ...
        for( int k = head[bridge ^ 1];; )
        {
            ...
            if( a == MF_TERMINAL )
            {
                if( terminal[k] < bottleneck )
                    bottleneck = terminal[k];
                break;
            }
            if( cap[a ^ 1] < bottleneck )
                bottleneck = cap[a ^ 1];
            k = head[a];
        }
        ...
        cap[bridge ^ 1] += bottleneck;
        cap[bridge] -= bottleneck;
        for( int k = head[bridge ^ 1];; )
        {
            int a = parent[k];
            if( a == MF_TERMINAL )
            {
                terminal[k] -= bottleneck;
                if( terminal[k] == 0 )
                    ORPHAN(k);
                break;
            }
        ...
        while( ok && orphan_head < orphans.length )
        {
            ...
            {
                if( (in_sink ? cap[a] : cap[a ^ 1]) <= 0 )
                    continue;
                int j = head[a];
                if( sink[j] != in_sink || parent[j] == MF_NONE )
                    continue;
                int d = 0, k = j;
                for( ;; )
                {
                ...
                for( k = j; stamp[k] != time; k = head[parent[k]] )
                {
                    stamp[k] = time;
                    distance[k] = d--;
                }
            ...
            for( int a = first[o]; a != MF_NONE; a = next[a] )
            {
                ...
                if( (in_sink ? cap[a] : cap[a ^ 1]) > 0 )
                    SET_ACTIVE(j);
                int up = parent[j];
                if( up != MF_TERMINAL && up != MF_ORPHAN && head[up] == o )
                    ORPHAN(j);
            }
        }
        orphans.length = 0;
...
double
bb_maxflow(
    int nodes, int edges, const double* terminals, const int* edge_nodes, const double* edge_caps)
{
    Graph g;
    double flow = -1;
    ...
    {
        for( int i = 0; i < nodes; i++ )
            graph_tweights(&g, i, terminals[i * 2], terminals[i * 2 + 1]);
...
typedef struct
{
    double weight[COMPONENTS], mean[COMPONENTS * 3], inverse[COMPONENTS * 9], log_norm[COMPONENTS];
} Gmm;

/* Symmetric 3×3 cyclic Jacobi with a fixed sweep count; returns the principal eigenpair. */
static double
largest_eigenvector(const double m[6], double vector[3])
{
    double a[3][3] = {
        { m[0], m[1], m[2] },
        { m[1], m[3], m[4] },
...
static void
statistics(const double* color, const int* members, int count, double mean[3], double cov[6])
{
    double r = 0, g = 0, b = 0;
...
static int
orchard_bouman(const double* color, const int* pixels, int count, int* component)
{
    ...
    while( clusters < COMPONENTS )
    {
        ...
        for( int c = 0; c < clusters; c++ )
        {
            ...
            if( value > best_value )
            {
                best = c;
                best_value = value;
                memcpy(best_axis, axis, sizeof axis);
                memcpy(best_mean, mean, sizeof mean);
            }
        }
        if( best < 0 || best_value <= 1e-9 )
            break;
        double split =
            best_mean[0] * best_axis[0] + best_mean[1] * best_axis[1] + best_mean[2] * best_axis[2];
        int* segment = order + start[best];
        int low = 0, high = 0;
        for( int k = 0; k < length[best]; k++ )
        {
            int p = segment[k];
            double projection = color[p * 3] * best_axis[0] + color[p * 3 + 1] * best_axis[1] +
                                color[p * 3 + 2] * best_axis[2];
            if( projection <= split )
                segment[low++] = p;
            else
                scratch[high++] = p;
        ...
        for( int c = clusters; c > best + 1; c-- )
        {
            start[c] = start[c - 1];
            length[c] = length[c - 1];
        }
        start[best + 1] = start[best] + low;
        length[best + 1] = high;
        length[best] = low;
        clusters++;
    }
...
static int
learn_gmm(const double* color, const int* pixels, int count, const int* component, Gmm* gmm)
{
    memset(gmm, 0, sizeof *gmm);
    int sizes[COMPONENTS] = { 0 }, offsets[COMPONENTS] = { 0 };
    for( int k = 0; k < count; k++ )
        sizes[component[pixels[k]]]++;
    ...
    for( int k = 0; k < count; k++ )
        grouped[cursor[component[pixels[k]]]++] = pixels[k];
    for( int c = 0; c < COMPONENTS; c++ )
    {
        ...
        double inv[9] = { e * i - f * f, d * f - b * i, b * f - d * e, d * f - b * i, a * i - d * d,
                          b * d - a * f, b * f - d * e, b * d - a * f, a * e - b * b };
        for( int k = 0; k < 9; k++ )
            gmm->inverse[c * 9 + k] = inv[k] / det;
...
static double
component_log_likelihood(const Gmm* gmm, int c, double r, double g, double b)
{
    if( gmm->weight[c] <= 0 )
        return -INFINITY;
...
static double
mixture_negative_log(const Gmm* gmm, double r, double g, double b)
{
    double values[COMPONENTS], max = -INFINITY, sum = 0;
    for( int c = 0; c < COMPONENTS; c++ )
    {
        values[c] = component_log_likelihood(gmm, c, r, g, b);
        if( values[c] > max )
            max = values[c];
    }
    if( max == -INFINITY )
        return 1e3;
    for( int c = 0; c < COMPONENTS; c++ )
        if( values[c] > -INFINITY )
            sum += exp(values[c] - max);
    double value = -(max + log(sum));
    return value < 1e3 ? value : 1e3;
}

static const int NEIGHBOURS[4][2] = {
    { 1,  0 },
    { 0,  1 },
    { 1,  1 },
    { -1, 1 }
...
static int
is_foreground(uint8_t label)
{
    return label == GC_FGD || label == GC_PR_FGD;
}

static int
split_labels(const uint8_t* labels, int n, int* fg, int* fg_count, int* bg, int* bg_count)
{
    *fg_count = 0;
    *bg_count = 0;
    for( int p = 0; p < n; p++ )
    {
        if( is_foreground(labels[p]) )
            fg[(*fg_count)++] = p;
        else
            bg[(*bg_count)++] = p;
    }
...
/* Runs GrabCut in place on `labels`. `rgb` is interleaved 0..255 sRGB. Returns 0 on allocation
 * failure. */
static int
grabcut(const double* rgb, int width, int height, uint8_t* labels, int iterations)
{
    ...
    for( int y = 0; y < height; y++ )
        for( int x = 0; x < width; x++ )
            for( int k = 0; k < 4; k++ )
            {
                ...
                double dr = rgb[p] - rgb[q], dg = rgb[p + 1] - rgb[q + 1],
                       db = rgb[p + 2] - rgb[q + 2];
    ...
    for( int y = 0; y < height; y++ )
        for( int x = 0; x < width; x++ )
            for( int k = 0; k < 4; k++ )
            {
                ...
                double dr = rgb[p] - rgb[q], dg = rgb[p + 1] - rgb[q + 1],
                       db = rgb[p + 2] - rgb[q + 2];
                smoothness[(y * width + x) * 4 + k] =
                    GAMMA * weights[k] * exp(-beta * (dr * dr + dg * dg + db * db));
            }
    if( !split_labels(labels, n, fg, &fg_count, bg, &bg_count) )
    {
        ok = 1;
        goto done;
    }
    if( !orchard_bouman(rgb, fg, fg_count, component) ||
        !orchard_bouman(rgb, bg, bg_count, component) )
        goto done;
    if( !learn_gmm(rgb, fg, fg_count, component, &fg_gmm) ||
        !learn_gmm(rgb, bg, bg_count, component, &bg_gmm) )
        goto done;
    ...
    for( int iteration = 0; iteration < iterations; iteration++ )
    {
        if( iteration > 0 )
        {
            for( int p = 0; p < n; p++ )
            {
                ...
                for( int c = 0; c < COMPONENTS; c++ )
                {
                    double value = component_log_likelihood(
                        gmm, c, rgb[p * 3], rgb[p * 3 + 1], rgb[p * 3 + 2]);
                    if( value > best_value )
                    {
                        best_value = value;
                        best = c;
        ...
        for( int p = 0; p < n; p++ )
        {
            ...
            if( label == GC_BGD )
                graph_tweights(&graph, p, 0, LAMBDA);
            else if( label == GC_FGD )
                graph_tweights(&graph, p, LAMBDA, 0);
            else
            {
                ...
                graph_tweights(
                    &graph,
                    p,
                    mixture_negative_log(&bg_gmm, r, g, b),
                    mixture_negative_log(&fg_gmm, r, g, b));
            }
        }
        for( int y = 0; y < height; y++ )
            for( int x = 0; x < width; x++ )
        ...
        if( flowed )
            for( int p = 0; p < n; p++ )
                if( labels[p] == GC_PR_BGD || labels[p] == GC_PR_FGD )
                    labels[p] = graph_in_source(&graph, p) ? GC_PR_FGD : GC_PR_BGD;
        graph_free(&graph);
        if( !flowed )
            goto done;
    }
...
int
bb_grabcut(const double* rgb, int width, int height, uint8_t* labels, int iterations)
{
...
static int
box_mean(const double* input, int width, int height, int radius, double* out)
{
    int stride = width + 1;
    ...
    for( int y = 0; y < height; y++ )
    {
        int y0 = y - radius < 0 ? 0 : y - radius,
            y1 = y + radius + 1 > height ? height : y + radius + 1;
        for( int x = 0; x < width; x++ )
        {
            int x0 = x - radius < 0 ? 0 : x - radius,
                x1 = x + radius + 1 > width ? width : x + radius + 1;
            out[y * width + x] = (table[y1 * stride + x1] - table[y0 * stride + x1] -
...
#define MEAN_OF(expr, target)                                                                      \
    do                                                                                             \
    {                                                                                              \
        for( int p = 0; p < n; p++ )                                                               \
            tmp[p] = (expr);                                                                       \
        if( !box_mean(tmp, width, height, radius, PLANE(target)) )                                 \
            goto done;                                                                             \
    } while( 0 )
    MEAN_OF(guide[p * 3], MR);
    MEAN_OF(guide[p * 3 + 1], MG);
    MEAN_OF(guide[p * 3 + 2], MB);
    MEAN_OF(input[p], MP);
    MEAN_OF((double)guide[p * 3] * guide[p * 3], RR);
    MEAN_OF((double)guide[p * 3] * guide[p * 3 + 1], RG);
    MEAN_OF((double)guide[p * 3] * guide[p * 3 + 2], RB);
    MEAN_OF((double)guide[p * 3 + 1] * guide[p * 3 + 1], GG);
    MEAN_OF((double)guide[p * 3 + 1] * guide[p * 3 + 2], GB);
    MEAN_OF((double)guide[p * 3 + 2] * guide[p * 3 + 2], BB);
    MEAN_OF((double)guide[p * 3] * input[p], RP);
    MEAN_OF((double)guide[p * 3 + 1] * input[p], GP);
    MEAN_OF((double)guide[p * 3 + 2] * input[p], BP);
    {
        double *mr = PLANE(MR), *mg = PLANE(MG), *mb = PLANE(MB), *mp = PLANE(MP);
        double *rr = PLANE(RR), *rg = PLANE(RG), *rb = PLANE(RB), *gg = PLANE(GG), *gb = PLANE(GB),
               *bb = PLANE(BB);
        double *rp = PLANE(RP), *gp = PLANE(GP), *bp = PLANE(BP), *ar = PLANE(AR), *ag = PLANE(AG),
               *ab = PLANE(AB), *b = PLANE(B0);
        for( int p = 0; p < n; p++ )
        {
            double vrr = rr[p] - mr[p] * mr[p] + epsilon, vrg = rg[p] - mr[p] * mg[p],
                   vrb = rb[p] - mr[p] * mb[p];
            double vgg = gg[p] - mg[p] * mg[p] + epsilon, vgb = gb[p] - mg[p] * mb[p],
                   vbb = bb[p] - mb[p] * mb[p] + epsilon;
            double cr = rp[p] - mr[p] * mp[p], cg = gp[p] - mg[p] * mp[p],
                   cb = bp[p] - mb[p] * mp[p];
            double i00 = vgg * vbb - vgb * vgb, i01 = vgb * vrb - vrg * vbb,
                   i02 = vrg * vgb - vgg * vrb;
            double i11 = vrr * vbb - vrb * vrb, i12 = vrb * vrg - vrr * vgb,
                   i22 = vrr * vgg - vrg * vrg;
        ...
        if( !box_mean(ar, width, height, radius, rr) || !box_mean(ag, width, height, radius, rg) ||
            !box_mean(ab, width, height, radius, rb) || !box_mean(b, width, height, radius, gg) )
            goto done;
        for( int p = 0; p < n; p++ )
            out[p] = (float)(rr[p] * guide[p * 3] + rg[p] * guide[p * 3 + 1] +
                             rb[p] * guide[p * 3 + 2] + gg[p]);
    }
    ok = 1;
done:
#undef MEAN_OF
...
static int
refine_mask(
    const float* guide, const float* binary, int width, int height, double feather, float* alpha)
{
    ...
    int ok = 0;
    if( !filtered || !binary_d || !coverage )
        goto done;
    if( !guided_filter(guide, binary, width, height, radius, 1e-4, filtered) )
        goto done;
    for( int p = 0; p < n; p++ )
        binary_d[p] = binary[p];
    if( !box_mean(binary_d, width, height, radius, coverage) )
        goto done;
    for( int p = 0; p < n; p++ )
    {
        if( coverage[p] <= 0 || coverage[p] >= 1 )
        {
            alpha[p] = binary[p];
            continue;
        }
...
static int
reduce_weighted(const Level* p, Level* out)
{
    int width = (p->width + 1) / 2, height = (p->height + 1) / 2;
    static const int kernel[3] = { 1, 2, 1 };
    out->width = width;
    out->height = height;
    out->data = ALLOC(float, width* height * 3);
    out->weight = ALLOC(float, width* height);
    if( !out->data || !out->weight )
        return 0;
    for( int y = 0; y < height; y++ )
        for( int x = 0; x < width; x++ )
        {
            ...
            for( int j = -1; j <= 1; j++ )
                for( int i = -1; i <= 1; i++ )
                {
                    int sx = clampi(x * 2 + i, 0, p->width - 1),
                        sy = clampi(y * 2 + j, 0, p->height - 1), k = kernel[i + 1] * kernel[j + 1];
                    int s = sy * p->width + sx;
            ...
            if( w > 0 )
            {
                out->data[o * 3] = (float)(r / w);
                out->data[o * 3 + 1] = (float)(g / w);
                out->data[o * 3 + 2] = (float)(b / w);
            }
        }
    return 1;
}
 ...
 */
static int
push_pull(const float* color, const float* weight, int width, int height, float* result)
{
    Level levels[40];
    int count = 1, ok = 0;
    levels[0].data = (float*)color;
    levels[0].weight = (float*)weight;
    levels[0].width = width;
    levels[0].height = height;
    while( (levels[count - 1].width > 1 || levels[count - 1].height > 1) && count < 40 )
    {
        if( !reduce_weighted(&levels[count - 1], &levels[count]) )
        {
            count++;
            goto done;
        }
    ...
    for( int l = count - 2; l >= 0; l-- )
    {
        ...
        for( int y = 0; y < level->height; y++ )
            for( int x = 0; x < level->width; x++ )
            {
                ...
                int x0 = clampi((int)floor(sx), 0, coarse->width - 1),
                    y0 = clampi((int)floor(sy), 0, coarse->height - 1);
                int x1 = x0 + 1 < coarse->width - 1 ? x0 + 1 : coarse->width - 1,
                    y1 = y0 + 1 < coarse->height - 1 ? y0 + 1 : coarse->height - 1;
                double fx = clampd(sx - x0, 0, 1), fy = clampd(sy - y0, 0, 1);
                int p = y * level->width + x;
                double w = level->weight[p] < 1 ? level->weight[p] : 1;
                for( int ch = 0; ch < 3; ch++ )
                {
                    double up = (filled[(y0 * coarse->width + x0) * 3 + ch] * (1 - fx) +
                                 filled[(y0 * coarse->width + x1) * 3 + ch] * fx) *
                                    (1 - fy) +
                                (filled[(y1 * coarse->width + x0) * 3 + ch] * (1 - fx) +
                                 filled[(y1 * coarse->width + x1) * 3 + ch] * fx) *
                                    fy;
    ...
    if( count == 1 )
        memcpy(result, color, sizeof(float) * (size_t)width * height * 3);
    ok = 1;
done:
    for( int l = 1; l < count; l++ )
    {
        free(levels[l].data);
        free(levels[l].weight);
    }
    return ok;
}
...
int
bb_push_pull(const float* color, const float* weight, int width, int height, float* result)
{
...
decontaminate(
    ...
    int height,
    double strength,
    float* foreground_delta,
    float* background_delta)
{
    int n = width * height, ok = 0;
    float *bg_weight = ALLOC(float, n), *fg_weight = ALLOC(float, n),
          *background = ALLOC(float, n * 3), *foreground = ALLOC(float, n * 3);
    if( !bg_weight || !fg_weight || !background || !foreground )
        goto done;
    for( int p = 0; p < n; p++ )
    {
        bg_weight[p] = alpha[p] <= 0.02f ? 1 : 0;
        fg_weight[p] = alpha[p] >= 0.98f ? 1 : 0;
    }
    if( !push_pull(color, bg_weight, width, height, background) ||
        !push_pull(color, fg_weight, width, height, foreground) )
        goto done;
    ...
    for( int p = 0; p < n; p++ )
    {
        ...
        if( a <= 0 )
            continue;
        for( int ch = 0; ch < 3; ch++ )
            background_delta[p * 3 + ch] =
                (float)((double)background[p * 3 + ch] - color[p * 3 + ch]);
        if( a >= 1 )
            continue;
    ...
    float* background_delta)
{
    ...
    if( !work_rgb || !binary || !guide || !color )
        goto done;

    bb_progress(0);
    /* Area-averaged downsample to the working size. */
    for( int ty = 0; ty < work_height; ty++ )
        for( int tx = 0; tx < work_width; tx++ )
        {
            int x0 = (int)floor((double)tx * width / work_width),
                x1 = (int)floor((double)(tx + 1) * width / work_width);
            int y0 = (int)floor((double)ty * height / work_height),
                y1 = (int)floor((double)(ty + 1) * height / work_height);
            if( x1 < x0 + 1 )
                x1 = x0 + 1;
            if( y1 < y0 + 1 )
                y1 = y0 + 1;
            ...
            work_rgb[o] = r / count;
            work_rgb[o + 1] = g / count;
            work_rgb[o + 2] = b / count;
        }
    if( !grabcut(work_rgb, work_width, work_height, work_labels, iterations) )
        goto done;
    ...
    for( int y = 0; y < height; y++ )
        for( int x = 0; x < width; x++ )
        {
            ...
            int x1 = x0 + 1 < work_width - 1 ? x0 + 1 : work_width - 1,
                y1 = y0 + 1 < work_height - 1 ? y0 + 1 : work_height - 1;
            double fx = sx - x0, fy = sy - y0;
#define FG(xx, yy) (is_foreground(work_labels[(yy) * work_width + (xx)]) ? 1.0 : 0.0)
            double value = (FG(x0, y0) * (1 - fx) + FG(x1, y0) * fx) * (1 - fy) +
                           (FG(x0, y1) * (1 - fx) + FG(x1, y1) * fx) * fy;
#undef FG
    ...
    if( !refine_mask(guide, binary, width, height, feather, alpha) )
        goto done;
    if( full_labels )
        for( int p = 0; p < n; p++ )
        {
            if( full_labels[p] == GC_FGD )
                alpha[p] = 1;
            else if( full_labels[p] == GC_BGD )
                alpha[p] = 0;
        }

    bb_progress(2);
    if( !decontaminate(color, alpha, width, height, strength, foreground_delta, background_delta) )
        goto done;
    ok = 1;
done:
    free(work_rgb);
    ...
    for( int y = 0; y < h; y++ )
        for( int x = 0; x < nw; x++ )
            for( int ch = 0; ch < c; ch++ )
            {
                double sum = 0;
                for( int k = -2; k <= 2; k++ )
                    sum += (double)p->data[(y * w + clampi(x * 2 + k, 0, w - 1)) * c + ch] *
                           kernel[k + 2];
    ...
    for( int y = 0; y < nh; y++ )
        for( int x = 0; x < nw; x++ )
            for( int ch = 0; ch < c; ch++ )
            {
                double sum = 0;
                for( int k = -2; k <= 2; k++ )
                    sum += (double)temp[(clampi(y * 2 + k, 0, h - 1) * nw + x) * c + ch] *
                           kernel[k + 2];
...
static float*
plane_expand(const Plane* p, int width, int height)
{
    ...
    for( int y = 0; y < height; y++ )
        for( int x = 0; x < width; x++ )
        {
            ...
            int x0 = (int)floor(sx), y0 = (int)floor(sy), x1 = x0 + 1 < w - 1 ? x0 + 1 : w - 1,
                y1 = y0 + 1 < h - 1 ? y0 + 1 : h - 1;
            double fx = sx - x0, fy = sy - y0;
            for( int ch = 0; ch < c; ch++ )
                out[(y * width + x) * c + ch] =
                    (float)((p->data[(y0 * w + x0) * c + ch] * (1 - fx) +
                             p->data[(y0 * w + x1) * c + ch] * fx) *
                                (1 - fy) +
                            (p->data[(y1 * w + x0) * c + ch] * (1 - fx) +
                             p->data[(y1 * w + x1) * c + ch] * fx) *
                                fy);
        }
    return out;
}
...
static Bounds
seam_bounds(int w, double position, double width)
{
    Bounds b;
    b.half = width * w / 2;
    ...
    if( upper < lo )
        upper = lo;
    b.lo = lo;
    b.hi = upper < hi ? upper : hi;
    return b;
}

/* Transition-centre cost: crossed edge, detail density, washed detail under the band, faded
 * protected alpha. */
static float*
seam_cost(
    const float* source,
    ...
    int* n_out)
{
    ...
    if( radius < 2 )
        radius = 2;
    float *luma = ALLOC(float, w* h), *edge = ALLOC(float, w* h), *detail = ALLOC(float, w* h),
          *cost = ALLOC(float, n* h);
    double *row_sum = ALLOC(double, w + 1), *fade = ALLOC(double, 2 * band + 1),
           *gate = ALLOC(double, band + 1);
    if( !luma || !edge || !detail || !cost || !row_sum || !fade || !gate )
    {
        free(cost);
        cost = NULL;
        goto done;
    ...
    for( int p = 0; p < w * h; p++ )
        luma[p] = (float)pow(
            source[p * 3] * 0.2126 + source[p * 3 + 1] * 0.7152 + source[p * 3 + 2] * 0.0722,
            1 / 2.2);
    for( int y = 0; y < h; y++ )
        for( int x = 0; x < w; x++ )
        {
...
#undef AT
    ...
    for( int y = 0; y < h; y++ )
    {
        ...
        for( int x = 0; x < w; x++ )
        {
            ...
            detail[y * w + x] =
                (float)((row_sum[b] - row_sum[a]) / ((double)(b - a) * (2 * radius + 1)));
        }
    }
    for( int k = -band; k <= band; k++ )
        fade[k + band] = smooth(-half, half, k);
    for( int k = 0; k <= band; k++ )
        gate[k] = 1 - foreground_gate(k + 0.5, half);
    ...
    for( int y = 0; y < h; y++ )
        for( int j = 0; j < n; j++ )
        {
            ...
            for( int k = -band; k <= band; k += 2 )
            {
                int xx = x + k;
                if( xx >= 0 && xx < w )
                    washed += detail[row + xx] * fade[k + band];
            }
            if( subject_alpha )
                for( int k = 0; k <= band; k++ )
                {
                    int xx = x + k;
                    if( xx < w )
                        faded += subject_alpha[row + xx] * gate[k];
                }
            double prior = (x - center) / (SEAM_REACH * w);
            prior *= prior;
            cost[y * n + j] =
                (float)(3 * (double)edge[row + x] + 4 * (double)detail[row + x] +
                        2 * washed / (band + 1) + 40 * faded / (band + 1) + 0.06 * prior);
        }
    *x0_out = x0;
    *n_out = n;
done:
    ...
    float* result)
{
    ...
    for( int y = 0; y < h; y++ )
        result[y] = (float)straight;
    if( !content_aware && !subject_alpha )
        return 1;
    int x0, n, ok = 0;
    ...
    if( !cost || !prev || !next || !back || !smoothed )
        goto done;
    ...
    for( int y = 1; y < h; y++ )
    {
        for( int j = 0; j < n; j++ )
        {
            double best = prev[j];
            int direction = 0;
            if( j > 0 && prev[j - 1] + bend < best )
            {
                best = prev[j - 1] + bend;
                direction = -1;
            }
            if( j + 1 < n && prev[j + 1] + bend < best )
            {
                best = prev[j + 1] + bend;
                direction = 1;
            }
    ...
    for( int y = 0; y < h; y++ )
    {
        ...
        {
            double weight = exp(-(double)(k * k) / (2 * sigma * sigma));
            sum += result[clampi(y + k, 0, h - 1)] * weight;
            total += weight;
        }
...
static float*
multiband(const float* source, const float* mask, int w, int h, const double surface[3])
{
    Plane images[7], masks[7];
    int levels = 1;
    float* result = NULL;
    images[0].data = (float*)source;
    images[0].width = w;
    images[0].height = h;
    images[0].channels = 3;
    masks[0].data = (float*)mask;
    masks[0].width = w;
    masks[0].height = h;
    masks[0].channels = 1;
    while( levels < 7 && images[levels - 1].height > 4 )
    {
        if( !plane_reduce(&images[levels - 1], &images[levels]) )
            goto done;
        if( !plane_reduce(&masks[levels - 1], &masks[levels]) )
        {
            free(images[levels].data);
            goto done;
        }
        levels++;
    }
    ...
    if( !reconstructed.data )
        goto done;
    for( int k = 0; k < count; k++ )
        reconstructed.data[k] =
            (float)(((double)images[i].data[k] - surface[k % 3]) * masks[i].data[k / 3] +
                    surface[k % 3]);
    for( i--; i >= 0; i-- )
    {
        ...
        reconstructed.data = up;
        reconstructed.width = current->width;
        reconstructed.height = current->height;
        if( !lower || !up )
        {
            free(lower);
            free(up);
        ...
        for( int k = 0; k < size; k++ )
            up[k] = (float)(up[k] + ((double)current->data[k] - lower[k]) * masks[i].data[k / 3]);
        free(lower);
    }
    ...
    int h)
{
    ...
    for( int y = 1; y < h - 1; y++ )
        ...
        {
            int p = y * w + x;
            if( distance[p] < -half * 1.3 || distance[p] > half )
                continue;
            inside[p] = 1;
            ...
            for( int ch = 0; ch < 3; ch++ )
            {
                ...
                {
                    int q = neighbours[k];
                    divergence += ((double)source[p * 3 + ch] - source[q * 3 + ch]) *
                                  ((double)mask[p] + mask[q]) / 2;
                }
    ...
    for( int iteration = 0; iteration < 180; iteration++ )
        ...
        {
            for( int y = 1; y < h - 1; y++ )
                for( int x = 1 + ((1 + y + parity) % 2); x < w - 1; x += 2 )
                {
                    int i = y * w + x;
                    if( !inside[i] )
                        continue;
                    ...
                    for( int ch = 0; ch < 3; ch++ )
                        out[p + ch] =
                            (float)(((double)out[p - 3 + ch] + out[p + 3 + ch] +
                                     out[p - w * 3 + ch] + out[p + w * 3 + ch] + rhs[p + ch]) /
                                    (4 + screen));
                }
        }
    free(rhs);
...
bb_blend(
    ...
    int surface_b,
    const float* subject_alpha,
    const float* foreground_delta,
    const float* background_delta,
    uint8_t* output,
    float* path_out)
{
    init_linear();
    int n = w * h, ok = 0;
    double surface[3] = { linear_from_srgb(surface_r),
                          linear_from_srgb(surface_g),
                          linear_from_srgb(surface_b) };
    float *source = ALLOC(float, n * 3), *path = ALLOC(float, h), *distance = ALLOC(float, n);
    float *background = NULL, *mask = ALLOC(float, n), *initial = ALLOC(float, n * 3),
          *blended = NULL;
    if( !source || !path || !distance || !mask || !initial )
        goto done;
    for( int p = 0; p < n; p++ )
        for( int ch = 0; ch < 3; ch++ )
            source[p * 3 + ch] = LINEAR[rgba[p * 4 + ch]];
    if( !find_seam(source, w, h, position, width, content_aware, subject_alpha, path) )
        goto done;
    if( path_out )
        memcpy(path_out, path, sizeof(float) * (size_t)h);
    Bounds bounds = seam_bounds(w, position, width);
    ...
    if( subject_alpha )
    {
        ...
        if( !background )
            goto done;
        for( int k = 0; k < n * 3; k++ )
            background[k] = (float)((double)source[k] + background_delta[k]);
    }
    for( int p = 0; p < n; p++ )
    {
        mask[p] = (float)(1 - smooth(-half, half, distance[p]));
        for( int ch = 0; ch < 3; ch++ )
            initial[p * 3 + ch] = (float)((double)background[p * 3 + ch] * mask[p] +
                                          surface[ch] * (1 - (double)mask[p]));
    }
    blended = method == 0   ? multiband(background, mask, w, h, surface)
              : method == 1 ? poisson(background, initial, mask, distance, half, w, h)
                            : initial;
    if( !blended )
        goto done;
    for( int p = 0; p < n; p++ )
    {
        ...
        for( int ch = 0; ch < 3; ch++ )
        {
            ...
            output[p * 4 + ch] =
                (uint8_t)srgb_from_linear(value * (1 - leave) + surface[ch] * leave);
        }
        output[p * 4 + 3] = 255;
    }
    ok = 1;
done:
    if( blended && blended != initial )
        free(blended);
