...
function referenceMaxflow(n: number, capacity: number[][], s: number, t: number): number {
	...
	for (;;) {
		...
		while (queue.length && parent[t] < 0) {
			const u = queue.shift()!;
			for (let v = 0; v < n; v++) if (parent[v] < 0 && residual[u][v] > 1e-12) { parent[v] = u; queue.push(v); }
		}
		if (parent[t] < 0) return flow;
		let bottleneck = Infinity;
		for (let v = t; v !== s; v = parent[v]) bottleneck = Math.min(bottleneck, residual[parent[v]][v]);
		for (let v = t; v !== s; v = parent[v]) { residual[parent[v]][v] -= bottleneck; residual[v][parent[v]] += bottleneck; }
		flow += bottleneck;
	}
}
...
const rectProtection = (x: number, y: number, w: number, h: number): BannerProtection => ({ source: '/art.jpg', rect: { x, y, width: w, height: h }, strokes: [] });

test('GrabCut recovers a subject from a rough rectangle and is deterministic', () => {
	const width = 96, height = 64, { rgba, truth } = synthetic(width, height);
	const run = () => {
	...
	let wrong = 0;
	for (let p = 0; p < truth.length; p++) if (((labels[p] === GC_FGD || labels[p] === GC_PR_FGD) ? 1 : 0) !== truth[p]) wrong++;
	assert.ok(wrong / truth.length < 0.02, `misclassified ${(100 * wrong / truth.length).toFixed(2)}%`);
});

test('correction strokes are hard constraints and scale with resolution', () => {
	const protection: BannerProtection = { source: '/a.jpg', rect: { x: 0.25, y: 0.25, width: 0.5, height: 0.5 },
		strokes: [{ label: 'foreground', radius: 0.05, points: [0.1, 0.1] }, { label: 'background', radius: 0.05, points: [0.5, 0.5] }] };
	for (const [w, h] of [[40, 30], [400, 300]]) {
		const labels = protectionLabels(protection, w, h);
		assert.equal(labels[Math.floor(0.1 * h) * w + Math.floor(0.1 * w)], GC_FGD);
		assert.equal(labels[Math.floor(0.5 * h) * w + Math.floor(0.5 * w)], GC_BGD);
		assert.equal(labels[Math.floor(0.3 * h) * w + Math.floor(0.3 * w)], GC_PR_FGD);
		assert.equal(labels[Math.floor(0.9 * h) * w + Math.floor(0.9 * w)], GC_BGD);
	}
});

test('guided filter preserves constants and keeps an edge-aligned mask sharp outside a narrow band', () => {
	const width = 40, height = 20, guide = new Float32Array(width * height * 3), binary = new Float32Array(width * height);
	for (let y = 0; y < height; y++) for (let x = 0; x < width; x++) {
...
test('decontamination recovers foreground color and is safe at alpha extremes', () => {
	const width = 64, height = 8, n = width * height;
	const F = [0.8, 0.2, 0.1], B = [0.05, 0.3, 0.9];
	...
	for (let p = 0; p < n; p++) {
		for (let c = 0; c < 3; c++) {
			const k = p * 3 + c;
			assert.ok(Number.isFinite(foregroundDelta[k]) && Number.isFinite(backgroundDelta[k]));
			const f = color[k] + foregroundDelta[k];
			assert.ok(f >= -1e-6 && f <= 1 + 1e-6, 'no out-of-range (over-saturated) colors');
			if (alpha[p] === 0) { assert.equal(foregroundDelta[k], 0); assert.equal(backgroundDelta[k], 0); }
			if (alpha[p] === 1) assert.equal(foregroundDelta[k], 0);
			if (alpha[p] >= 0.25 && alpha[p] < 1) assert.ok(Math.abs(f - F[c]) < 0.02, `recovered F at alpha ${alpha[p]}`);
		}
	}
...
test('subject-preserving composite keeps protected pixels, reaches the exact surface, and is deterministic', () => {
	const w = 320, h = 48, rgba = new Uint8ClampedArray(w * h * 4);
	for (let y = 0; y < h; y++) for (let x = 0; x < w; x++) {
		const p = (y * w + x) * 4;
		rgba[p] = (x * 13 + y * 7) % 256; rgba[p + 1] = (x * 3 + y * 19) % 256; rgba[p + 2] = (x * 17 + y * 5) % 256; rgba[p + 3] = 255;
	}
	const alpha = new Float32Array(w * h);
	for (let y = 0; y < h; y++) for (let x = 120; x < 170; x++) alpha[y * w + x] = 1;
	const subject: SubjectFrame = { alpha, foregroundDelta: new Float32Array(w * h * 3), backgroundDelta: new Float32Array(w * h * 3) };
...
test('config migration keeps v1 artifacts and generation drops selections for other artwork', () => {
	const legacy = normalizeBannerBlendConfig({ method: 'poisson', contentAware: true, position: 0.49, width: 0.13, surface: '#f2e9e6' } as never);
	...
	const config = { ...legacy, protectSubject: true, protection: { ...protection, source: 'http://a.test/images/art_crop/1.jpg' } };
	assert.equal(configForGeneration(config, 'http://b.test/images/art_crop/1.jpg').protection?.source, 'http://a.test/images/art_crop/1.jpg');
	assert.equal(configForGeneration(config, 'http://b.test/images/art_crop/2.jpg').protection, null);
