...
function descriptor(input,shear=0,blur=0){
 ...
 const norm=Math.hypot.apply(null,v)||1,pnorm=Math.hypot.apply(null,proj)||1;for(let i=0;i<D;i++)v[i]/=norm;for(let i=0;i<W;i++)proj[i]/=pnorm;return{v,proj};
}
...
window.scanFontOCR=async({blur=1.3,rectify=false,fontFile="beleren.woff"}={})=>{
 ...
 try{for(const line of await findTitleStrips(image)){
  ...
  for(const band of bands){
   ...
   for(const shear of [0])for(const hit of lookup(band.canvas,index,shear)){if(hit.score>(byName.get(hit.name)?.score??-1))byName.set(hit.name,{name:hit.name,score:hit.score,shear});}
   const candidates=Array.from(byName.values()).sort((a,b)=>b.score-a.score).slice(0,5),poly=[[0,0],[band.canvas.width,0],[band.canvas.width,band.canvas.height],[0,band.canvas.height]].map(([x,y])=>strip.toImage(x+band.left,y+band.top));outputs.push({poly:plane?poly.map(p=>plane.toOriginal(p[0],p[1])):poly,candidates});console.log('Font match',outputs.length,JSON.stringify(candidates.slice(0,2)));
  }
