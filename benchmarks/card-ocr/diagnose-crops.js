...
window.diagnoseCrops=async({boxes,model='small'})=>{
 ...
 try{for(const {id,box:[x,y,w,h]}of boxes){
  const cx=x+w/2,cy=y+h/2,near=lines.slice().sort((a,b)=>{const dist=l=>{const yc=l.y1+(cx-l.x1)*Math.tan(l.angle);return Math.abs(cy-yc)+Math.max(0,l.x1-cx,cx-l.x2)*2;};return dist(a)-dist(b);})[0],angle=near.angle,c=Math.cos(angle),s=Math.sin(angle),hh=Math.max(15,h-w*Math.abs(Math.tan(angle))+8),ww=w/c;
