...
export async function scanReferenceTitles({
  ...
  const start = performance.now(),
    allReferences = await (
      await fetch(
        "/models/title-references/" +
          (suppliedRows ? "catalog.json" : "manifest.json"),
      )
    ).json(),
    needed = suppliedRows
  ...
  for (const ref of manifest) {
    ...
    let response = await fetch("/models/title-references/" + ref.file);
    if (
      !response.ok ||
    ...
    for (const crop of crops.slice(0, 2)) {
      if (crop.canvas.width < ref.name.replace(/\s/g, "").length * 3) continue;
      ...
      templates.get(ref.name).push({
        ...
        frame: ref.frame,
        id: ref.id,
        box: [crop.left, crop.top, crop.canvas.width, crop.canvas.height],
      });
    }
    bitmap.close();
    onProgress({
  ...
  const src = cv.imread(full),
    target = cv.matFromArray(4, 1, cv.CV_32FC2, [0, 0, 96, 0, 96, 16, 0, 16]),
    rows =
      suppliedRows ||
      (
        await (
...
window.scanReferenceTitles = scanReferenceTitles;
