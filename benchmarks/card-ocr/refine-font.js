...
function normalize(v){const n=Math.hypot.apply(null,v)||1;return v.map(x=>x/n);}
...
export async function refineFontMatches(image,rough,{broadBlur=false}={}){
 ...
 try{for(const r of rough){
  ...
  const candidates=r.candidates.map(candidate=>{let score=-1,blur;const raw=template(candidate.name);for(const sx of (broadBlur?[.25,.5,.8,1.2,1.6,2,2.5,3]:[.25,.5,.8,1.2,1.6,2]))for(const scale of (broadBlur?[.6,1,1.6]:[1])){const sy=Math.max(.25,Math.min(broadBlur?6:4,sx*ratio*scale)),v=blurred(raw,sx,sy);let s=0;for(let i=0;i<D;i++)s+=v[i]*query[i];if(s>score){score=s;blur=[sx,sy];}}return Object.assign({},candidate,{coarseScore:candidate.score,score,blur});}).sort((a,b)=>b.score-a.score);
  results.push(Object.assign({},r,{candidates}));source.delete();M.delete();out.delete();
 }
 return results;
