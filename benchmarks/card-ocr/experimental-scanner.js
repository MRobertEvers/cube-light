...
export async function scanExperimental({
  ...
  if (!navigator.gpu)
    throw Error("The experimental verifier requires a browser with WebGPU.");
  ...
  try {
    ...
    const glmRows = distinct(
      rough.outputs.filter((r) => r.candidates[0].score >= 0.5 && eligible(r)),
    ).slice(0, 60);
    ...
    if (glmRows.length) {
      ...
      passes.push(glm);
      candidates = addTextConsensus(candidates, glm.outputs, rough.outputs);
...
window.scanExperimental = scanExperimental;
