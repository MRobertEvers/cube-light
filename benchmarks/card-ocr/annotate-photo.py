...
add('Melded Moxite',[[1108,309,114,34]])
add('Melded Moxite',[[990,304,43,13],[1036,303,25,13],[1060,310,42,14]],'partial')
...
Path(__file__).with_name('photo-ground-truth.json').write_text(json.dumps({'annotationRevision':2,'correction':'User confirmed the upper-right pile is Melded Moxite.','image':'IMG_8535.jpeg','width':5712,'height':4284,'notes':'Full = sufficient exposed title to identify independently. Partial = identity annotated using visible art and adjacent cards; must not imply OCR can recover the hidden suffix. Unreadable left-edge stack, partially exposed Unsummon at right and completely hidden cards excluded from known-name scoring; any detections there require review. Exact physical total is indeterminate.','cards':rows},indent=2)+'\n')
